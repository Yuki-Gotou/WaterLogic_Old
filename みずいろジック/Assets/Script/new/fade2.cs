    ...
    void Update()
    {
        switch (fadeStart)
        {
            case "FadeIn":
                ...
                alpha.r = (Time.time - CurrentFadeTime) / (fadeTime / 1.5f);
                alpha.g = (Time.time - CurrentFadeTime) / (fadeTime / 1.5f);
                alpha.b = (Time.time - CurrentFadeTime) / (fadeTime / 1.5f);
                if (alpha.a < 0.0f)
                {
                    alpha.a = 0.0f;
                }
            ...
            case "FadeOut":
                ...
                alpha.r = 1.0f - (Time.time - CurrentFadeTime) / (fadeTime * 2);
                alpha.g = 1.0f - (Time.time - CurrentFadeTime) / (fadeTime * 2);
                alpha.b = 1.0f - (Time.time - CurrentFadeTime) / (fadeTime * 2);
