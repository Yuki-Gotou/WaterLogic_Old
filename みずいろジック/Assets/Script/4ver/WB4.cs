...
public class WB4 : MonoBehaviour {
    public float XScale;
    ...
    void Start()
    {
        ...
        transform.localScale = new Vector3(transform.localScale.x, NowYScale + GameObject.Find("緑4").transform.localScale.y, transform.localScale.z);
    ...
    {
        //変化があったら入る
        if (/*ChengeScale != 0.0f|| WG1.ChangeF==1.0f||*/WLB4.DF != 0)
        {
            NowChangeY = (ChengeScale / XScale);
            ChengeScale = 0.0f;
            FuturNowYScale = NowYScale + NowChangeY;
            FuturYScale = NowYScale + NowChangeY + WG4.NowChangeY;
            WG4.ChangeF = 0.0f;
            ChangeF = 1.0f;
        }
        if (GF == 1)
        {
            ...
            if (NowYScale < FuturYScale)
            {
                ...
                Debug.Log("青4まだ", gameObject);
                Debug.Log(NowYScale);
                Debug.Log(FuturNowYScale);
                if (NowYScale > FuturNowYScale)
                {
                    Debug.Log("青4PLUS", gameObject);
                    Debug.Log(NowYScale);
                    Debug.Log(FuturNowYScale);
                    //緑に開始の合図
                    WG4.GF = 1;
