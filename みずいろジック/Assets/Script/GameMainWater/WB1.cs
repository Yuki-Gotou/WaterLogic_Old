...
public class WB1 : MonoBehaviour {
    public float XScale;
    ...
    void Update () {
        //変化があったら入る
        if (/*ChengeScale != 0.0f|| WG1.ChangeF==1.0f||*/WLB1.DF!=0)
        {
            NowChangeY = (ChengeScale / XScale);
            ChengeScale = 0.0f;
            FuturNowYScale = NowYScale + NowChangeY;
            FuturYScale = NowYScale + NowChangeY+WG1.NowChangeY;
            WG1.ChangeF = 0.0f;
            ChangeF = 1.0f;
        }
        if (GF == 1)
        {
            ...
            if (NowYScale < FuturYScale)
            {
                ...
                Debug.Log("青1まだ", gameObject);
                Debug.Log(NowYScale);
                Debug.Log(FuturNowYScale);
                if (NowYScale > FuturNowYScale)
                {
                    Debug.Log("青1PLUS", gameObject);
                    Debug.Log(NowYScale);
                    Debug.Log(FuturNowYScale);
                    //緑に開始の合図
                    WG1.GF = 1;
